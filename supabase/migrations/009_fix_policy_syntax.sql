-- Fix materialized view policy syntax error
...
-- 2. Create a regular table instead of materialized view to support embedded resources
DROP MATERIALIZED VIEW IF EXISTS dde_texts_with_commentary;

CREATE TABLE IF NOT EXISTS dde_texts_with_commentary AS
SELECT 
  t.id,
  t.doc_id,
  t.commentary_id,
...
-- 4. Enable RLS on the table (now this will work)
ALTER TABLE dde_texts_with_commentary ENABLE ROW LEVEL SECURITY;

-- 5. Create policy for public read access (correct syntax for tables)
