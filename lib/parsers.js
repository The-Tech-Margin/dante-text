 ...
 */
function parseTextFile(filePath, commId, cantica, cantoNum, usedDocIds = new Set()) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    const segments = [];
    ...
    for (let i = 1; i < rawSegments.length; i += 2) {
      ...
      if (segmentText) {
        const segment = parseSegmentHeader(segmentHeader, segmentText, commId, cantica, cantoNum, usedDocIds);
        if (segment) {
    ...
    if (segments.length === 0 && content.trim()) {
      const fallbackSegment = {
        docId: generateDocId(commId, cantica, cantoNum, 0, 0, usedDocIds),
        commId,
 ...
 */
function parseSegmentHeader(header, content, commId, cantica, cantoNum, usedDocIds = new Set()) {
  const segment = {
    commId,
    cantica: mapCantica(cantica),
    cantoId: parseInt(cantoNum) || 0,
...
/**
 * Generate doc_id using original 12-character system with uniqueness guarantee
 * Format: cccccannlllt
 * @param {string} commId - 5-char commentary ID
 * @param {string} cantica - Cantica abbreviation
 * @param {string} cantoNum - Canto number
 * @param {number} startLine - Starting line number
 * @param {number} baseTieBreaker - Base tie-breaker digit
 * @param {Set} usedDocIds - Set to track used doc_ids (passed from migration script)
 * @returns {string} 12-character doc_id
 */
function generateDocId(commId, cantica, cantoNum, startLine, baseTieBreaker = 0, usedDocIds = new Set()) {
  ...
  let tieBreaker = baseTieBreaker;
  let docId;
  
  // Keep incrementing tie-breaker until we get a unique doc_id
  do {
    const tie = String(tieBreaker % 10); // Keep it single digit
    docId = commId + canticaId + canto + line + tie;
    tieBreaker++;
  } while (usedDocIds.has(docId) && tieBreaker < 10);
  
  // If still not unique after 10 tries, use a hash-based approach
  if (usedDocIds.has(docId)) {
    const contentHash = (commId + cantica + cantoNum + startLine).split('').reduce((hash, char) => 
      ((hash << 5) - hash + char.charCodeAt(0)) & 0x7fff, 0);
    const tie = String(contentHash % 10);
    docId = commId + canticaId + canto + line + tie;
  }
