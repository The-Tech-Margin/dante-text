...
function parseSegmentHeader(header, content, commId, cantica, cantoNum) {
  ...
  // Generate doc_id using original system (duplicates now allowed)
 ...
 */
function generateDocId(commId, cantica, cantoNum, startLine, baseTieBreaker = 0) {
  ...
  const tie = String(baseTieBreaker);
